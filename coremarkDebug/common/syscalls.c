...
static void vprintfmt(void (*putch)(int, void**), void **putdat, const char *fmt, va_list ap)
{
  ...
  int base, lflag, width, precision, altflag, fp_pos;
  char padc;
...
#if (HAS_FLOAT == 1)  
    // (signed) float, to 0.00001 precision
    case 'f':
      ...
      //unsigned long long frac_part = (unsigned long long)(pow(10, fp_pos)*(fnum - (double)int_part));
      for (int i=0; i<fp_pos; i++) {
      	frac_part *= 10;
      }
      //print int part
      printnum(putch, putdat, int_part, 10, width, padc);
      putch('.', putdat);
      printnum(putch, putdat, frac_part, 10, fp_pos, padc);
